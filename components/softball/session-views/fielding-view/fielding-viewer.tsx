...
export default function SoftballFieldingView({
  ...
  const getUserName = (userId: string) => displayName(userMap.get(userId)?.profiles ?? null);
                    ...
                    assignments: Object.fromEntries(
                      ALL_POSITIONS.map((p) => [
                        ...
                        getEffectiveAssignment(
                          data,
                          typeof selectedInning === "number" ? selectedInning : 1,
                          p.key,
                        ),
                      ]),
                    ),
                  ...
                  <TableBody>
                    {innings.map((inning) => {
                      ...
                      return (
                        ...
                        >
                          <TableCell className="font-mono text-xs">{inning}</TableCell>
                          <TableCell>
                            {positions.length > 0 ? positions.map((p) => p.label).join(", ") : "—"}
                          </TableCell>
                        </TableRow>
                      );
                    })}
