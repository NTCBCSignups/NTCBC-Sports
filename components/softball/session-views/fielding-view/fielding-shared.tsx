  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      <svg viewBox="0 0 100 100" overflow="visible" className="w-full h-auto">
        ...
        />
        {/* Bases */}
        <rect x="48.5" y="84.5" width="3" height="3" className="fill-muted-foreground/60" />{" "}
        {/* Home */}
        <rect
          x="76.5"
          y="60.5"
  ...
  return (
    <div className={editorLayout ? "pr-6" : "overflow-x-auto"}>
      <table className="w-max border-collapse text-xs">
        <thead>
          <tr>
            <th className={cn(stickyClass, "py-1 text-left font-medium w-20")}>Inning</th>
            {innings.map((inning) => (
              <th
                key={inning}
                className="px-1 py-1 text-center font-medium min-w-[120px] snap-start"
              >
                {inning}
              </th>
            ))}
            ...
            <Fragment key={group.label}>
              <tr>
                <td
                  className={cn(
                    stickyClass,
                    "pt-2 pb-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground whitespace-nowrap",
                  )}
                >
                  {group.label}
                </td>
