...
type OrderEntry =
  | { type: "player"; signup: SignupRow }
  | { type: "spacer"; id: string; label: string };
      ...
      <Table>
        ...
        <TableBody className="[&_tr:last-child]:border-b">
          {group.entries.map((entry, i) => {
            ...
            return (
              <TableRow key={signup.id} className={isCurrentUser ? "bg-status-info" : ""}>
                <TableCell className="font-mono text-xs">{startIndex + i + 1}</TableCell>
                <TableCell className="px-0 align-middle">
                  {teamMemberIds.has(signup.user_id) && <TeamMemberBadge />}
                </TableCell>
                <TableCell>{displayName(signup.profiles)}</TableCell>
              </TableRow>
            );
          })}
        </TableBody>
    ...
    <Table>
      ...
      <TableBody>
        {entries.map((entry) => {
          ...
          return (
            <TableRow key={signup.id} className={isCurrentUser ? "bg-status-info" : ""}>
              <TableCell className="font-mono text-xs">{playerIndex}</TableCell>
              <TableCell className="px-0 align-middle">
                {teamMemberIds.has(signup.user_id) && <TeamMemberBadge />}
              </TableCell>
              <TableCell>{displayName(signup.profiles)}</TableCell>
...
function OrderedContent({
  ...
  if (nonEmptyGroups.length === 1 && nonEmptyGroups[0]!.entries.length > 1) {
    ...
    const leftGroup: Group = {
      label: nonEmptyGroups[0]!.label,
          ...
          <div className="flex-1 min-w-0">
            {rightGroups.map((group, gi) => (
              <ColumnTable
                ...
                startIndex={
                  leftEntries.length +
                  rightGroups.slice(0, gi).reduce((sum, g) => sum + g.entries.length, 0)
                }
                currentUserId={currentUserId}
                teamMemberIds={teamMemberIds}
              />
 ...
 */
export function CustomOrderedEditor({ signups, viewData, ref }: SessionViewEditorProps) {
  const currentOrder = Array.isArray(viewData) ? (viewData as string[]) : [];
  const confirmed = signups.filter((s) => s.status === "confirmed");

  type EditorItem =
    | { type: "player"; signup: (typeof confirmed)[0] }
    | { type: "spacer"; id: string; label: string };
  ...
  const buildInitialOrder = () => {
    ...
    for (const entry of currentOrder) {
      ...
      if (isSpacer(entry)) {
        const spacerItem: EditorItem = {
          type: "spacer",
          id: entry,
          label: getSpacerLabel(entry),
        };
        if (pastDivider) {
  ...
  const itemToKey = (item: EditorItem) => (item.type === "spacer" ? item.id : item.signup.user_id);
  ...
  const updateSpacerLabel = (section: "visible" | "hidden", index: number, label: string) => {
    if (section === "visible") {
      setVisibleItems((prev) =>
        prev.map((item, i) => (i === index && item.type === "spacer" ? { ...item, label } : item)),
      );
    } else {
      setHiddenItems((prev) =>
        prev.map((item, i) => (i === index && item.type === "spacer" ? { ...item, label } : item)),
  ...
  if (confirmed.length === 0) {
    return <p className="text-sm text-muted-foreground py-4">No confirmed sign-ups to reorder.</p>;
  }
