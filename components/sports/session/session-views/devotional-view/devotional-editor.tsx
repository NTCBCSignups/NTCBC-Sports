...
function SectionEditor({
    ...
    const toggleAllLines = useCallback(() => {
        ...
        const allHidden =
            ...
            section.items.every((i) => i.facilitatorOnly);
        ...
        >
            ...
            <div className="flex items-center gap-2 px-3 py-2 border-b bg-muted/30">
                ...
                <span className="text-sm shrink-0">
                    {SECTION_META[section.type].emoji}
                </span>

                <Input
                    value={
                        section.customTitle ??
                        SECTION_META[section.type].defaultTitle
                    }
                    onChange={(e) =>
                        onMetaChange({ customTitle: e.target.value })
                    }
            ...
            {section.type === "passage" && (
                <div className="px-3 pt-2">
                    <Input
                        ...
                        onChange={(e) =>
                            onMetaChange({ passageReference: e.target.value })
                        }
...
export default function DevotionalEditor({
    ...
    const initialData =
        (viewData as DevotionalViewData | null) ?? createDefaultDevotionalData();
    ...
    const updateSection = useCallback(
        (sectionId: string, updates: Partial<DevotionalSection>) => {
            setData((prev) => ({
                ...prev,
...
function AddSectionPicker({
    ...
    onAdd: (type: DevotionalSectionType) => void;
}) {
    return (
        <Select
            value=""
            onValueChange={(val) => onAdd(val as DevotionalSectionType)}
        >
            ...
            <SelectContent>
                {SECTION_TYPE_OPTIONS.map((type) => (
                    <SelectItem key={type} value={type}>
                        {SECTION_META[type].emoji}{" "}
                        {SECTION_META[type].defaultTitle}
                    </SelectItem>
                ))}
            </SelectContent>
        </Select>
