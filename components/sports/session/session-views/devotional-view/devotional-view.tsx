...
const BULLET_STYLE = ["list-disc", "list-disc", "list-[circle]", "list-[square]", "list-[square]"] as const;
        ...
        return (
            <div
                className={cn(
                    ...
                    isFacilitatorOnly &&
                        "border-l-2 border-dashed border-primary/40 pl-2",
                )}
            >
    ...
    return (
        ...
        >
            <ul
                className={cn(
                    "list-outside pl-5",
                    INDENT_CLASSES[item.indent] ?? "ml-14",
                )}
            >
                <li
                    className={cn(
                        BULLET_STYLE[item.indent] ?? "list-[square]",
                    )}
                >
...
function DevotionalSectionView({
    ...
    const subtitle =
        section.type === "passage" && section.passageReference
            ? ` - ${section.passageReference}`
            : "";
...
export default function DevotionalView({
    ...
    const hasVisibleContent =
        ...
        data.sections.some((s) =>
            s.items.some((item) => !item.facilitatorOnly),
        );
