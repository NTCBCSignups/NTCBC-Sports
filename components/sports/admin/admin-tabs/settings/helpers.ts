...
import { SETTINGS_TAB_ID } from "@/config/admin-tab-metadata";
...
import { ACCESS_LEVEL_OPTIONS, ROLE_OPTIONS } from "./constants";
import type {
  AdminTabDraft,
  EditableAdminTab,
  EditableTab,
...
export function createAdminTabDraft(tabId?: string): AdminTabDraft {
  const fallbackDefinition =
    ADMIN_TAB_DEFINITIONS.find((definition) => definition.id !== SETTINGS_TAB_ID) ??
...
export function summarizePermissions(permissions: EditableTabPermissions): string {
  return ACCESS_LEVEL_OPTIONS.map(
    (option) => `${option.label}: ${roleLabel(permissions[option.value])}`,
  ).join(" | ");
}
