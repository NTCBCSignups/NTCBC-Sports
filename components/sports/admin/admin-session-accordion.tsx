...
export default function SessionAccordion({
  ...
  if (sessions.length === 0) {
    return <p className="text-sm text-muted-foreground py-4">No sessions.</p>;
  }
  ...
  return (
    <Accordion type="multiple" className="space-y-2">
      {sessions.map((session) => {
        ...
        const activeCount = sessionSignups.filter((s) => s.status === "confirmed").length;
        const tab = getResolvedTab(config, session.session_type);
        const sessionTypeLabel = tab.defaultTitlePrefix ?? tab.label;
                ...
                <div className="flex shrink-0 flex-col items-end gap-1.5 sm:gap-2">
                  ...
                  {session.status === SESSION_STATUS.cancelled ? (
                    ...
                    <Badge variant="outline" className="text-xs">
                      {activeCount}
                      {session.player_cap ? ` / ${session.player_cap}` : ""}
                    </Badge>
                  )}
                </div>
              </div>
